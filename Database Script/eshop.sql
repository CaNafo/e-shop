...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('COMMENT') and o.name = 'FK_COMMENT_HAS_COMME_PRODUCTS')
alter table COMMENT
   drop constraint FK_COMMENT_HAS_COMME_PRODUCTS
go

if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('COMMENT') and o.name = 'FK_COMMENT_IS_COMMEN_USERS')
alter table COMMENT
   drop constraint FK_COMMENT_IS_COMMEN_USERS
go
...
if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('PRODUCT_RATING') and o.name = 'FK_PRODUCT__HAS_RATIN_PRODUCTS')
alter table PRODUCT_RATING
   drop constraint FK_PRODUCT__HAS_RATIN_PRODUCTS
go

if exists (select 1
   from sys.sysreferences r join sys.sysobjects o on (o.id = r.constid and o.type = 'F')
   where r.fkeyid = object_id('PRODUCT_RATING') and o.name = 'FK_PRODUCT__RATED_PRO_USERS')
alter table PRODUCT_RATING
   drop constraint FK_PRODUCT__RATED_PRO_USERS
go
...
if exists (select 1
           ...
           where  id    = object_id('COMMENT')
            and   name  = 'IS_COMMENTING_FK'
            and   indid > 0
            and   indid < 255)
   drop index COMMENT.IS_COMMENTING_FK
go

if exists (select 1
            from  sysindexes
           where  id    = object_id('COMMENT')
            and   name  = 'HAS_COMMENT_FK'
            and   indid > 0
            and   indid < 255)
   drop index COMMENT.HAS_COMMENT_FK
go
...
if exists (select 1
            from  sysindexes
           where  id    = object_id('PRODUCT_RATING')
            and   name  = 'HAS_RATING_FK'
            and   indid > 0
            and   indid < 255)
   drop index PRODUCT_RATING.HAS_RATING_FK
go

if exists (select 1
            from  sysindexes
           where  id    = object_id('PRODUCT_RATING')
            and   name  = 'RATED_PRODUCT_FK'
            and   indid > 0
            and   indid < 255)
   drop index PRODUCT_RATING.RATED_PRODUCT_FK
go
...
create table COMMENT (
   ...
   constraint PK_COMMENT primary key (USER_ID, CATEGORY_ID, PRODUCT_ID, COMMENT_ID)
)
go

/*==============================================================*/
/* Index: HAS_COMMENT_FK                                        */
/*==============================================================*/




create nonclustered index HAS_COMMENT_FK on COMMENT (CATEGORY_ID ASC,
  PRODUCT_ID ASC)
go

/*==============================================================*/
/* Index: IS_COMMENTING_FK                                      */
/*==============================================================*/




create nonclustered index IS_COMMENTING_FK on COMMENT (USER_ID ASC)
go
...
/*==============================================================*/
/* Index: RATED_PRODUCT_FK                                      */
/*==============================================================*/




create nonclustered index RATED_PRODUCT_FK on PRODUCT_RATING (USER_ID ASC)
go
...
create nonclustered index HAS_RATING_FK on PRODUCT_RATING (CATEGORY_ID ASC,
  PRODUCT_ID ASC)
go
...
alter table COMMENT
   add constraint FK_COMMENT_HAS_COMME_PRODUCTS foreign key (CATEGORY_ID, PRODUCT_ID)
      references PRODUCTS (CATEGORY_ID, PRODUCT_ID)
go

alter table COMMENT
   add constraint FK_COMMENT_IS_COMMEN_USERS foreign key (USER_ID)
      references USERS (USER_ID)
go
...
alter table PRODUCT_RATING
   add constraint FK_PRODUCT__HAS_RATIN_PRODUCTS foreign key (CATEGORY_ID, PRODUCT_ID)
      references PRODUCTS (CATEGORY_ID, PRODUCT_ID)
go

alter table PRODUCT_RATING
   add constraint FK_PRODUCT__RATED_PRO_USERS foreign key (USER_ID)
      references USERS (USER_ID)
go
